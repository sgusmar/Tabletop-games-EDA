...
def read_ids_from_csv(path: Path) -> List[str]:
    """Leer IDs numéricos desde la primera columna de un CSV.

    Omite filas vacías y entradas no numéricas en la primera columna.
    Devuelve una lista de cadenas con los IDs listos para consultar la
...
def chunks(iterable: List[str], size: int) -> Generator[List[str], None, None]:
    """Generador que devuelve sublistas (chunks) de tamaño `size`.

    Utilidad sencilla para agrupar listas largas de IDs en lotes más
...
def load_processed(path: Path) -> set:
    """Cargar IDs ya procesados desde un archivo de registro.

    El archivo debe contener un ID por línea. Si el archivo no existe,
    devuelve un conjunto vacío para empezar desde cero.
    """
    if not path.exists():
        return set()
...
def append_processed(path: Path, ids: Iterable[str]) -> None:
    """Añadir IDs procesados al registro (uno por línea).

    Crea el directorio padre si no existe y añade las nuevas entradas al
    final del archivo para permitir reanudar la ejecución posteriormente.
    """
...
def _normalized_numplayers_result(value: str | None) -> str | None:
    if value is None:
        return None
    # Normalizar etiquetas comunes de las encuestas a claves estables.
...
def parse_collection(ratings: ET.Element) -> List[Dict[str, Any]]:
    ...
    for category in ("owned", "trading", "wanting", "wishing"):
        ...
        collection.append(
            {
                "category": category,
                # Preferir el atributo 'value'; si no existe, usar el texto.
...
def parse_thing_xml_item(item: ET.Element) -> Dict[str, Any]:
    """Parsear un elemento `<item>` del XML de BGG a un diccionario.

    Extrae campos relevantes (nombre, descripción, jugadores mínimos/
    máximos, tiempos de juego, estadísticas, categorías, autores, etc.).

    Returns:
        Un diccionario con claves estables apto para serializar a JSON.
        Si el elemento no contiene un `id` válido, devuelve un dict vacío.
    """
    if item is None:
        return {}

    def get_text(elem: ET.Element) -> str | None:
        # Devuelve el atributo 'value' si existe; en caso contrario, el
        # contenido textual del elemento (recortado). Maneja elementos
        # faltantes devolviendo None.
        if elem is None:
            return None
        val = elem.get("value")
    ...
    # Conteos de jugadores: leer min/max y manejar elementos ausentes
    for tag in ("minplayers", "maxplayers"):
        elem = item.find(tag)
        entry[tag] = get_text(elem)
...
def parse_thing_xml(xml_text: str) -> List[Dict[str, Any]]:
    """Parsear la respuesta XML de `/thing` en una lista de entradas.

    La respuesta puede contener uno o varios elementos `<item>` cuando se
    solicitan múltiples IDs en una sola petición. Para respuestas que no
    sean XML válidas se devuelven entradas con la clave `raw_xml`.
    """
    try:
...
def fetch_with_retries(session: requests.Session, url: str, params: Dict[str, Any], max_retries: int = 6) -> str:
    """Realizar una petición GET con reintentos y backoff exponencial.

    Parámetros:
        session: `requests.Session` ya configurada (headers, timeouts...)
        url: URL de destino
        params: Parámetros de consulta
        max_retries: Número máximo de intentos antes de fallar

    Devuelve el texto de la respuesta si la petición tiene éxito. Lanza
    ...
    for attempt in range(max_retries):
        try:
            ...
            if status == 429:
                # 429 Too Many Requests: esperar exponencialmente
                wait = (2 ** attempt) * 5
            ...
            wait = (2 ** attempt) * 5
            print(f"  Error de petición: {e}. Esperando {wait}s y reintentando (intento {attempt+1}/{max_retries})")
            time.sleep(wait)
            continue
...
def process_batch(
    ...
    """Procesar un lote de IDs y guardar los resultados en JSON.

    - Agrupa los IDs en sub-lotes (`ids_per_request`) para cada petición.
    - Llama a la API, parsea la respuesta y acumula resultados.
    - Duerme `delay` segundos entre peticiones y registra los IDs
      procesados en `processed_path`.
    """
